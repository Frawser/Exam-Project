...
router.post("/", async (req, res) => {
  ...
  try {
    if (!userId || !exercise || !personalBest) {
      return res
        .status(400)
        .json({ message: "userId, exercise, and value are required" });
    }

    const newPersonalRecord = new PersonalRecord({
      userId,
      exercise,
      value: personalBest,
    });
...
router.get("/user/:userId", async (req, res) => {
  ...
  try {
    const personalRecords = await PersonalRecord.find({ userId }).sort({
      createdAt: -1,
    });
...
router.get("/latest/deadlift/:userId", async (req, res) => {
  ...
  try {
    const latestDeadlift = await PersonalRecord.findOne({
      exercise: "deadlift",
      userId,
    }).sort({ createdAt: -1 });
...
router.get("/latest/benchpress/:userId", async (req, res) => {
  ...
  try {
    const latestBenchpress = await PersonalRecord.findOne({
      exercise: "benchPress",
      userId,
    }).sort({ createdAt: -1 });
...
router.get("/latest/squat/:userId", async (req, res) => {
  ...
  try {
    const latestSquat = await PersonalRecord.findOne({
      exercise: "squat",
      userId,
    }).sort({ createdAt: -1 });
...
router.get("/first/deadlift/:userId", async (req, res) => {
  ...
  try {
    const firstDeadlift = await PersonalRecord.findOne({
      exercise: "deadlift",
      userId,
    }).sort({ createdAt: 1 });
...
router.get("/first/benchpress/:userId", async (req, res) => {
  ...
  try {
    const firstBenchpress = await PersonalRecord.findOne({
      exercise: "benchPress",
      userId,
    }).sort({ createdAt: 1 });
...
router.get("/first/squat/:userId", async (req, res) => {
  ...
  try {
    const firstSquat = await PersonalRecord.findOne({
      exercise: "squat",
      userId,
    }).sort({ createdAt: 1 });
    res.json(firstSquat);
